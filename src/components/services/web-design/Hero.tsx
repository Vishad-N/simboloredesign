...
export default function Hero({ data }: { data: ServiceData }) {
  return (
    <section className={`relative pt-32 pb-20 md:pt-40 md:pb-32 px-4 md:px-8 ${data.heroImage ? 'overflow-hidden' : ''}`}>
      ...
      {data.heroImage && <ServiceHeroImage image={data.heroImage} darkOverlay={false} bottomFade={false} />}

      <div className="max-w-[1320px] mx-auto text-right relative z-10">
        <ScrollReveal direction="left">
          ...
          <h1 className="font-heading text-5xl md:text-7xl font-extrabold tracking-tight leading-[1.1] mb-6 text-dark">
            Build a Stunning<br />Online Home.
          </h1>
        </ScrollReveal>

        <ScrollReveal direction="right">
          <p className="max-w-2xl ml-auto text-lg md:text-xl leading-relaxed text-dark">
            {data.overview}
          </p>
