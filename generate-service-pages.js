...
const heroTemplate = `import Image from "next/image";
...
const benefitsTemplate = `import ScrollReveal from "@/components/ui/ScrollReveal";
...
const processTemplate = `import ScrollReveal from "@/components/ui/ScrollReveal";
import LiquidCard from "@/components/ui/LiquidCard";
import { ServiceData } from "@/data/services";
...
const caseStudyTemplate = `import Image from "next/image";
import Link from "next/link";
...
function getPageTemplate(slug) {
  const isVideoEditing = slug === 'video-editing';
  const functionName = slug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('') + 'Page';
...
import Hero from "@/components/services/${slug}/Hero";
import Benefits from "@/components/services/${slug}/Benefits";
import Process from "@/components/services/${slug}/Process";
${isVideoEditing ? `import CaseStudy from "@/components/services/${slug}/CaseStudy";` : ''}
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        ${isVideoEditing ? `<CaseStudy data={serviceData} />` : ''}
        <ScrollReveal direction="left">
          <CTA />
        </ScrollReveal>
      </div>
...
slugs.forEach(slug => {
  const componentsDir = path.join(basePath, 'components', 'services', slug);
  const appDir = path.join(basePath, 'app', slug);

  fs.mkdirSync(componentsDir, { recursive: true });
  fs.mkdirSync(appDir, { recursive: true });

  fs.writeFileSync(path.join(componentsDir, 'Hero.tsx'), heroTemplate);
  fs.writeFileSync(path.join(componentsDir, 'Benefits.tsx'), benefitsTemplate);
  fs.writeFileSync(path.join(componentsDir, 'Process.tsx'), processTemplate);
  
  if (slug === 'video-editing') {
    fs.writeFileSync(path.join(componentsDir, 'CaseStudy.tsx'), caseStudyTemplate);
  }

  fs.writeFileSync(path.join(appDir, 'page.tsx'), getPageTemplate(slug));
});

console.log('Successfully generated service pages and components.');
